...
export const assignTenantAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      if (!user) return { ok: false as const, error: "No user with that email. Ask them to sign up first." };
      const { error: roleErr } = await supabaseAdmin.from("user_roles")
        .insert({ user_id: user.id, tenant_id: data.tenantId, role: "tenant_admin" });
      if (roleErr && !/duplicate|unique/i.test(roleErr.message)) return { ok: false as const, error: roleErr.message };
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const { error } = await supabaseAdmin.from("user_roles").delete()
        .eq("user_id", data.userId).eq("tenant_id", data.tenantId).eq("role", "tenant_admin");
      if (error) return { ok: false as const, error: error.message };
      const { data: remaining } = await supabaseAdmin.from("user_roles").select("id")
        .eq("user_id", data.userId).eq("role", "tenant_admin").limit(1);
...
export const listTenantAdmins = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      const { data: roles, error } = await supabaseAdmin.from("user_roles").select("user_id")
        .eq("tenant_id", data.tenantId).eq("role", "tenant_admin");
      ...
      const admins = await Promise.all(ids.map(async (id) => {
        const { data: u } = await supabaseAdmin.auth.admin.getUserById(id);
        const profile = profiles?.find((p) => p.id === id);
        return { user_id: id, email: u?.user?.email ?? "(unknown)", display_name: profile?.display_name ?? null };
      }));
